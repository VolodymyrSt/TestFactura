Shader "Custom/Tree_Wind_Instanced_Fixed"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            UNITY_INSTANCING_BUFFER_START(UnityPerMaterial)
                ...
                UNITY_DEFINE_INSTANCED_PROP(float,  _BranchRadius)      // нове
            ...
            float3 ApplyWind(float3 posOS, float4 vColor, float3 normalOS)
            {
                ...
                float3 originalPosOS = posOS; // для стабільного шуму листя
                ...
                float mask = saturate((currentHeight - (treeH * stiffness)) /
                ...
                // ── TRUNK (без змін, працює як зсув по висоті) ─────
                ...
                // ── BRANCH (виправлене гойдання, без розтягувань) ──
                ...
                float branchRadius   = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _BranchRadius);

                // Нелінійна функція: наростає швидко, потім плато – гілка не «видовжується»
                float bendFactor = 1.0 - exp(-distFromTrunk / max(0.01, branchRadius));
                float branchAngle = sin(branchTime) * branchBendAmt * branchMask;

                // Горизонтальне зміщення (однакове для всіх точок на кінчику)
                posOS.xz += windDir.xz * branchAngle * bendFactor;

                // Вертикальне просідання гілки – тепер не залежить від distFromTrunk,
                // а лише від поточної висоти, щоб уникнути викручування.
                float droopFactor = saturate(currentHeight / max(0.01, treeH));
                posOS.y -= abs(branchAngle) * mask * droopFactor * 0.2;

                // ── CROWN (листя) ──────────────────────────────────
                float flutter    = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _BranchFlutter);
                float leafWeight = vColor.g;

                // Стабільний шум на основі оригінальної позиції
                ...
                // ── CROWN FOLLOW (крона слідує за гілками) ────────
                float followStr   = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _CrownFollowStrength);
                // Використовуємо той самий branchAngle та bendFactor на максимумі (1.0)
                // Щоб крона прив’язувалась до кінчиків гілок.
                float crownFollow = sin(branchTime) * branchBendAmt * leafWeight * followStr;
                posOS.xz += windDir.xz * crownFollow;   // без distFromTrunk
                posOS.y  -= abs(crownFollow) * 0.1;
        ...
        Pass
        {
            ...
            UNITY_INSTANCING_BUFFER_START(UnityPerMaterial)
                ...
                UNITY_DEFINE_INSTANCED_PROP(float,  _BranchPhase)
                UNITY_DEFINE_INSTANCED_PROP(float,  _BranchFlutter)
                UNITY_DEFINE_INSTANCED_PROP(float,  _BranchRadius)
                UNITY_DEFINE_INSTANCED_PROP(float,  _TreeHeight)
                UNITY_DEFINE_INSTANCED_PROP(float,  _RootStiffness)
                UNITY_DEFINE_INSTANCED_PROP(float,  _WindPhase)
                UNITY_DEFINE_INSTANCED_PROP(float,  _ShimmerIntensity)
            ...
            float3 ApplyWind(float3 posOS, float4 vColor, float3 normalOS)
            {
                ...
                // Trunk wave — базовий хитання стовбура
                float trunkSin = sin(branchTime);
                float trunkWave;
                #if defined(_LEAN_WIND)
                    trunkWave = (tBend * leanWeight + trunkSin * tBend * (1.0 - leanWeight)) * mask;
                #else
                    trunkWave = trunkSin * tBend * mask;
                #endif

                // Branch — повторює trunk-фазу, але з власною амплітудою
                // branchMask замість bendFactor — вся гілка зміщується як ціле
                float branchAngle = sin(branchTime) * branchBendAmt * branchMask;

                // Єдиний XZ зсув: trunk + branch разом
                posOS.xz += windDir.xz * (trunkWave + branchAngle);

                // Вертикальне просідання від branchMask (не від висоти!)
                posOS.y -= abs(branchAngle) * branchMask * 0.15;
