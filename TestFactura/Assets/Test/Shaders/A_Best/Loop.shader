Shader "Custom/Tree_Wind_Instanced"
{
    Properties
    {
        ...
        _ShimmerSpeed  ("Shimmer Speed",        Float) = 8.0   // ← додано: контроль частоти тремтіння
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Fog.hlsl"   // ← додано
            ...
            UNITY_INSTANCING_BUFFER_START(UnityPerMaterial)
                ...
                UNITY_DEFINE_INSTANCED_PROP(float,  _ShimmerSpeed)          // ← додано
            ...
            float3 ApplyWind(float3 posOS, float4 vColor, float3 normalOS)
            {
                ...
                float distFromTrunk = length(posOS.xz);   // відстань після згинання стовбура – OK
                ...
                // ── 3. CROWN (листя, тремтіння) ──────────────────────
                float flutter           = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _BranchFlutter);
                float shimmerIntensity  = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _ShimmerIntensity);
                float shimmerSpeed      = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _ShimmerSpeed); // ← новий параметр
                float leafWeight        = vColor.g;
                float leafNoise         = frac(dot(posOS.xyz, float3(12.9898, 78.233, 45.164)));

                // Новий розрахунок shimmer з контрольованою частотою
                float shimmer = sin(time * shimmerSpeed + leafNoise * 20.0) * flutter * shimmerIntensity;
        ...
        Pass
        {
            ...
            // Така ж логіка вітру, як у Forward пасі
            float3 ApplyWind(float3 posOS, float4 vColor, float3 normalOS)
            {
                #if !defined(_USE_WIND_ON)
                    return posOS;
                ...
                float flutter           = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _BranchFlutter);
                float shimmerIntensity  = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _ShimmerIntensity);
                float shimmerSpeed      = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _ShimmerSpeed);
                float leafWeight        = vColor.g;
                float leafNoise         = frac(dot(posOS.xyz, float3(12.9898, 78.233, 45.164)));
            ...
            Varyings vertShadow(Attributes v)
            {
                ...
                float4 posWS    = TransformObjectToWorld(float4(animatedPos, 1));
