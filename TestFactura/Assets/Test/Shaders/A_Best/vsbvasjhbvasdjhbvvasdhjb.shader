Shader "Custom/TreeWindRGB"
{
    ...
    frac(
        sin(
            dot(floor(worldPos.xz * 0.25), float2(12.9898, 78.233))
        ) * 43758.5453
    );

float branchPrimary =
    sin(time * branchFrequency + branchNoise * 2.0)
    * branchStrength;

float branchSecondary =
    sin(time * (branchFrequency * 0.6) + branchNoise * 1.5)
    * branchStrength
    * branchTurbulence;

float branchFollow =
    trunkWave * crownFollow;
