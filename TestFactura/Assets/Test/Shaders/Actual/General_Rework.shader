    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 ApplyWind(float3 posOS, float4 vColor)
            {
                ...
                float branchBend =
                    sin(time * 1.3 + pivotWS.x * 0.3 + pivotWS.z * 0.3)
                    * (tBend * 0.5)
                    * branchWeight
                    * bendMask;
                ...
                float leafNoise =
                    frac(dot(posOS.xyz, float3(12.9898, 78.233, 45.164)));

                float shimmer =
                    sin(time * 14.0 + leafNoise * 20.0)
                    * flutter
                    * UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _ShimmerIntensity);

                float leafWave =
                    sin(time * 2.5 + leafNoise * 6.0)
                    * flutter;
