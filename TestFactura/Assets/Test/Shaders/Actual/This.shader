Shader "Custom/TreeWindAdvanced"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 ApplyWind(float3 posOS, float4 vColor)
            {
                ...
                float branchFreq      = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _BranchFrequency);
                ...
                float branchPrimary =
                    sin(time * branchFreq + noise * 4.0)
                    * branchStrength;

                float branchSecondary =
                    sin(time * (branchFreq * 2.1) + noise * 7.0)
                    * branchStrength
                    * branchTurbulence;
                ...
                posOS.xyz += leafOffset * bendMask;

                posOS.y += leafVertical;
