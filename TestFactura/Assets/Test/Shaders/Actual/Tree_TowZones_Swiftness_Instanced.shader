Shader "Custom/Tree_FullFeatures_Stiffness"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 ApplyWind(float3 posOS, float4 vColor)
            {
                ...
                // --- МАСКИ ТА ЖОРСТКІСТЬ ---
                float flexibility = saturate(1.0 - vColor.r); // Червоний канал = Stiffness
                flexibility = max(0.05, flexibility); // Мінімальний рух навіть для жорстких зон
                ...
                float trunkWave;
                    ...
                    trunkWave = (staticLean + dynamicSway) * mask * flexibility;
                #else
                    trunkWave = sin(time + pivotWS.x * 0.5) * tBend * mask * flexibility;
                #endif

                // --- ЛИСТЯ ТА ГІЛКИ (МЕРЕХТІННЯ) ---
                ...
                // G - гілки, B - листя. Flexibility впливає і на них.
                float finalLeafMotion = (branchWave + shimmer) * (/*vColor.g +*/ vColor.b) * flexibility;

                // --- РЕЗУЛЬТАТ ---
                float totalWind = (trunkWave * bendMask + finalLeafMotion) * localZoneStrength;
                posOS.xz += windDir.xz * totalWind;
                
                // Присідання залежить від сили нахилу та гнучкості
                posOS.y -= abs(totalWind) * 0.15 * bendMask * flexibility; 
            ...
            half4 frag(Varyings i) : SV_Target
            {
                ...
                // Колір вертексів (vColor) фарбує дерево в Blender-кольори
